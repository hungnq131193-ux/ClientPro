...
[![Version](https://img.shields.io/badge/version-1.3.4-blue.svg)](manifest.json)
...
- **Tên phát hành** — **Genesis** (tên phát hành công khai, không hiển thị trong
  UI app).
- **Phiên bản app (semver)** — hiện tại **`1.3.4`**. Số kỹ thuật nội bộ giữ cho
  tooling đồng bộ và tương thích, không hiển thị cho người dùng. Nguồn duy nhất:
  `package.json`.
- **cache-buster asset** — hiện tại **`AUDIT_CLEANUP_20260724`**. Nguồn: `ASSET_V`
  trong `sw.js`.
