...
[![Version](https://img.shields.io/badge/version-1.5.5-blue.svg)](manifest.json)
...
**1. Phiên bản app (semver)** — hiện tại **`1.5.5`**. **Nguồn duy nhất (single source of truth) là `package.json` → `version`.** Sửa ở đó rồi chạy:
...
**2. Tag cache-buster asset (chuỗi tự do)** — hiện tại **`PERFMAP_20260709`**, thường đặt theo mốc ngày/đợt redesign. Nguồn là `ASSET_V` trong `sw.js`; phải **đồng nhất** tại:
