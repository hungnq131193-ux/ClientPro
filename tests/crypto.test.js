...
test('lockApp: xóa sạch key + cache khỏi RAM và đưa app về trạng thái khóa', async () => {
  const { api, localStorage } = loadSecurity();
  localStorage.setItem('app_pin', 'sealed-dummy'); // PIN đã thiết lập -> được phép khóa
  await api.setMasterKey(api.generateMasterKey());
  const cipher = await api.encryptText('Nguyễn Văn A');

  assert.equal(api.isAppUnlocked(), true);
  assert.equal(api.decryptText(cipher), 'Nguyễn Văn A', 'cache nóng trước khi khóa');

  api.lockApp(); // showLockScreen ném lỗi vì DOM null -> đã được try/catch nuốt

  assert.equal(api.isAppUnlocked(), false, 'sau lockApp app phải ở trạng thái khóa');
  const st = api.getState();
  assert.equal(st.mk, null);
  assert.equal(st.legacy, null);
  assert.equal(st.hasGcmKey, false);
  assert.equal(api.decryptText(cipher), cipher, 'cache plaintext đã bị xóa -> fail-open trả ciphertext');
});

test('lockApp: CHƯA thiết lập PIN -> không khóa (tránh nhốt người dùng giữa setup)', async () => {
  const { api } = loadSecurity();
  await api.setMasterKey(api.generateMasterKey());
  api.lockApp();
  assert.equal(api.isAppUnlocked(), true, 'không có PIN_KEY thì lockApp phải là no-op');
});
