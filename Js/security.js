...
function encryptText(text) {
    ...
    try { return CryptoJS.AES.encrypt(String(text), masterKey).toString(); } catch (e) { return text; }
}
...
async function checkSecurity() {
    ...
    } catch (err) { console.log("Offline mode: Security check skipped."); }
}
...
async function saveSecuritySetup() {
    ...
    if (!ans) {
        const storedEmp = localStorage.getItem(EMPLOYEE_KEY);
        if (storedEmp) ans = storedEmp;
        else return alert("Nhập mã nhân viên");
    }
...
async function validatePin() {
    ...
    if (decrypted && decrypted.startsWith('mk_')) {
        ...
        setTimeout(() => { alert("Sai mã PIN"); clearPin(); }, 100);
    }
}
...
async function activateApp() {
    ...
    if (!key || !employeeId) return alert("Vui lòng nhập đầy đủ Mã kích hoạt và Mã nhân viên");
        ...
        if ((result && result.status && String(result.status).toLowerCase() === 'success') || String(result).toLowerCase().includes('success')) {
            ...
            if (!hasOldData) {
                ...
                if (decrypted && decrypted.startsWith('mk_')) {
                    ...
                    if (confirmDel) {
                        try { localStorage.clear(); indexedDB.deleteDatabase(DB_NAME); } catch (e) {}
                        masterKey = null;
                        localStorage.setItem(ACTIVATED_KEY, 'true');
                        localStorage.setItem(EMPLOYEE_KEY, employeeId);
                        const modal = getEl('activation-modal');
