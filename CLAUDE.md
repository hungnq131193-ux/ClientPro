...
- **Phiên bản**: 1.5.7 — nguồn duy nhất `package.json`, đồng bộ bằng `npm run sync:version` (§7).
...
**QUY TẮC FIELD CIPHER (bắt buộc — đúc từ chuỗi bug thật v1.5.5→1.5.7 từng làm hỏng dữ liệu vĩnh viễn)**:
- **R1 — Ghi**: `await encryptText(...)` trong `try/catch`, mã hóa trước transaction.
- **R2 — Hiển thị/populate**: kiểm `_looksEncrypted(v)` (định nghĩa gốc `05_customers.js`, nhận diện **CẢ** `cpg1:` lẫn `U2FsdGVk`) trước khi render/set `.value`. Cache-miss = "chưa sẵn sàng": không render ciphertext ra UI, không re-encrypt field đó khi lưu.
- **R3 — Input editable**: `await decryptFieldAsync` từng field trước khi điền; vẫn encrypted sau khi cố giải mã → để trống. Khi lưu: ô trống nhưng field gốc còn ciphertext → **giữ nguyên ciphertext gốc**, không ghi đè rỗng (đã áp dụng ở `_doSaveAsset`, `saveCustomerNotes`).
...
- **R5 — Field mã hóa mới**: đảm bảo nơi NẠP cache (`decryptFieldAsync` hoặc tương đương) tồn tại và thực sự được gọi trước khi component render — đừng giả định `primeFieldCache()` lo (bug thật v1.5.5: `decryptCustomerAssetsAsync` từng là dead code → TSBĐ hiện `cpg1:...` sau unlock).
...
MapLibre lazy-load (`MAPLIBRE_V` **phải = `ASSET_V`**), markers KH/TSBĐ theo lat/lng, clustering supercluster khi >100 điểm. `locateMe()`, `getCurrentGPS()` (Geolocation, đã whitelist trong Permissions-Policy/CSP).
...
**05**: CRUD (`saveCustomer`, `deleteCurrentCustomer`, `toggleCustomerStatus`); `_looksEncrypted` định nghĩa gốc ở đây; tìm kiếm **không dấu** `_normVi()` (lowercase + bỏ dấu NFD + đ→d) khớp tên + CCCD + SĐT (bỏ khoảng trắng), chỉ số `nName/nPhone/nCccd` cache trong `__custSummaryCache` (không tính lại mỗi keystroke). **Cố ý KHÔNG** dùng virtual list / viewport marker culling (over-engineering ở quy mô vài trăm KH; rAF chunk-render 25 + decrypt cache đủ mượt). `saveCustomerNotes()` có guard R3. `openFolder()`: sau khi hiện hồ sơ, chạy nền prime cache notes + TSBĐ (R5) rồi tự re-render tab info/assets đang mở. Approval workflow: `confirmApproval()`, `closeApproveModal()`.
**06**: CRUD TSBĐ gắn customer, hỗ trợ `priceRef` (tham khảo giá), lat/lng hiển thị trên map. `window.decryptCustomerAssetsAsync(customer, {batchSize})`: prime cache mọi field TSBĐ **kể cả `name`** theo batch (chỉ nạp cache, không render — caller quyết định). `openEditAssetModal(index)` **async** (R3). `_doSaveAsset()` guard R3. `_deepDecryptLabel()` dùng `_looksEncrypted` (R4).
**13**: selection mode — `toggleCustSelectionMode()`, `sendSelectedCustomersToUser()`, `deleteSelectedCustomers()`; dùng cho cloud transfer / approve hàng loạt.
...
- `_isCryptoJSCiphertext()` (07, quyết định folderName Drive/hiển thị) dùng `_looksEncrypted()` (R4).
...
- **v1.5.7**, `ASSET_V = CRYPTOFIX2_20260709`.
- Các đợt nâng cấp lớn đã hoàn tất: **P1** single-source versioning + siết CSP; **P2** Security Core AES-256-GCM + masterKey CSPRNG + migration resume-safe; **P3** accessibility (ModalA11y, pinch-zoom, reduced-motion); **P4** tìm kiếm không dấu + index cache; **P5** test suite zero-dep + E2E/axe/Lighthouse CI; chuẩn hóa error/loading toàn codebase (§6.7); reliability v1.5.4 (chống double-submit, persist ok-check, gate-trước-loader, camera lifecycle); v1.5.5 (marker clustering, lazy decrypt, ảnh mã hóa at-rest); v1.5.6–1.5.7 (chuỗi fix field cipher — đã đúc thành quy tắc **R1–R5 §5** và cạm bẫy CSS **§6.10**, không cần đọc lại lịch sử bug).
...
---
*Tài liệu sống — cập nhật cùng mỗi thay đổi lớn. Last updated: 2026-07-09 (ICT). Phiên bản skill: 2.0 (viết lại nén token; giữ nguyên toàn bộ kiến thức load-bearing, khử lặp changelog/case-study).*
