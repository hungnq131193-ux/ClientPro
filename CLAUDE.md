...
**Case study #2 — v1.5.7, sót field `asset.name` khi vá v1.5.6**: bản v1.5.6 vá 2 lỗ hổng chính ở trên nhưng vẫn sót field `asset.name` (tiêu đề card TSBĐ) — hiện `cpg1:...` với các bản ghi TSBĐ cũ đã bị migration GCM (`_reencryptRecord`) re-encrypt tên sang định dạng mới. Nguyên nhân gốc: `_deepDecryptLabel()` (06_assets.js, hàm giải mã riêng cho `name` vì field này về nguyên tắc KHÔNG được mã hóa ở lần ghi mới — xem `_doSaveAsset`) được viết từ thời điểm chỉ có 2 khả năng cho field này (plaintext mới, hoặc ciphertext CryptoJS legacy `U2FsdGVkX1...`) nên hard-code check đúng 1 tiền tố đó; và `decryptCustomerAssetsAsync()` (hàm prime cache sau khi mở hồ sơ) không có `"name"` trong danh sách field cần nạp. Cùng lỗi (hard-code 1 tiền tố) cũng có ở `_isCryptoJSCiphertext()` trong `07_drive.js` (dùng để né hiển thị/dùng ciphertext làm tên folder Drive). **Bài học**: KHÔNG tự viết check "chuỗi này có phải ciphertext không" bằng cách hard-code 1 tiền tố cụ thể (`U2FsdGVkX1` hay `cpg1:`) — luôn gọi `_looksEncrypted()` (định nghĩa gốc ở 05_customers.js, nhận diện CẢ 2 dạng) làm nguồn duy nhất, kể cả cho field "theo thiết kế không mã hóa" như `asset.name`, vì migration field cipher có thể re-encrypt BẤT KỲ field legacy nào (kể cả field đó) sang định dạng GCM mới bất cứ lúc nào mà logic hiển thị không hay biết.
...
- **Phiên bản**: 1.5.7 (ASSET_V: CRYPTOFIX2_20260709). Nguồn semver: `package.json` (dùng `npm run sync:version`).
- **Recent change (2026-07-09 — Bug fix v1.5.7, TSBĐ vẫn hiện `cpg1:...` sau v1.5.6)**: v1.5.6 đã vá lỗ hổng lazy-decrypt chính (thiếu nơi nạp cache + guard double-encryption) nhưng bỏ sót field `asset.name` (tên/tiêu đề card TSBĐ). Nguyên nhân: `renderAssets()` (06) dùng hàm nội bộ `_deepDecryptLabel()` để giải mã `asset.name` — hàm này viết từ thời `asset.name` CHỈ có 2 dạng có thể gặp (plaintext mới, hoặc ciphertext CryptoJS legacy `"U2FsdGVkX1..."`), nên chỉ strip đúng tiền tố đó rồi dừng. Nhưng migration GCM (`_reencryptRecord` trong 02_security.js, chạy 1 lần khi nâng cấp field cipher) re-encrypt MỌI field legacy — kể cả `name` của các bản ghi TSBĐ cũ (tạo trước khi có quyết định "không mã hóa tên TSBĐ") — sang định dạng mới `"cpg1:..."`. `_deepDecryptLabel()` không nhận diện được tiền tố `cpg1:` nên trả nguyên ciphertext, hiện thẳng ra card TSBĐ (tiêu đề bold ở đầu card). Đồng thời `window.decryptCustomerAssetsAsync()` (hàm prime `__fieldPlainCache` sau khi mở hồ sơ) cũng chưa từng nạp cache cho `name` — chỉ 7 field TSBĐ khác — nên kể cả khi dùng đúng `decryptText()` thì vẫn cache-miss ở lần mở đầu tiên. Fix: (1) `_deepDecryptLabel()` đổi điều kiện lặp sang `_looksEncrypted(s)` (nhận diện cả `cpg1:` lẫn `U2FsdGVk`) thay vì check cứng 1 tiền tố; (2) thêm `"name"` vào mảng `fields` của `decryptCustomerAssetsAsync()` để cache được prime đúng; (3) áp dụng cùng cách nhận diện `_looksEncrypted` cho phần "opportunistic persist plaintext" của `name` (trước chỉ trigger khi thấy tiền tố legacy). Bug tương tự cũng có ở `assets/07_drive.js`: `_isCryptoJSCiphertext()` (dùng để quyết định folderName Drive/hiển thị đã giải mã xong hay chưa) cũng chỉ nhận diện tiền tố legacy — sửa để dùng `_looksEncrypted()` (05) khi có sẵn, fallback check cả 2 tiền tố nếu không. **Bài học bổ sung cho case study §4.3**: mọi hàm helper tự chế để "nhận diện ciphertext" hoặc "unwrap ciphertext" phải dùng `_looksEncrypted()` làm nguồn duy nhất (nhận diện CẢ `cpg1:` và `U2FsdGVk`) — không tự viết check tiền tố cứng cục bộ, vì migration có thể chuyển bất kỳ field legacy nào (kể cả field "theo thiết kế không mã hóa" như `asset.name`) sang định dạng GCM mới bất cứ lúc nào.
...
*Last updated: 2026-07-09 (ICT) — Bug fix v1.5.7: tên TSBĐ vẫn hiện ciphertext `cpg1:...` trên card sau v1.5.6, do `_deepDecryptLabel()` (06_assets.js) và `_isCryptoJSCiphertext()` (07_drive.js) chỉ nhận diện tiền tố ciphertext legacy `U2FsdGVkX1`, bỏ sót định dạng GCM mới `cpg1:` mà migration tạo ra cho field `asset.name`; và `decryptCustomerAssetsAsync()` chưa prime cache cho field `name`. Fix: đổi mọi check sang dùng `_looksEncrypted()` (05_customers.js) làm nguồn duy nhất + thêm `name` vào danh sách field được prime cache (§8).*  
*Phiên bản skill: 1.6*
