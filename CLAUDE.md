...
| Phiên bản app (semver) | `1.1.0` | `package.json` (single source of truth — số kỹ thuật nội bộ, không hiển thị cho người dùng) |
| Cache-buster asset | `UXHARDEN_20260721` | `ASSET_V` trong `sw.js` |
...
| `pdf-toolkit/pdf_toolkit_utils.js` | Hàm thuần PDF Toolkit (parser khoảng trang, safe filename, signature, preset…) — unit-test được trong Node |
| `pdf-toolkit/pdf_toolkit_core.js` | Lõi toolkit: lazy-load vendor PDF, object URL registry, validate, download/share/ZIP, trừu tượng đọc PDF |
| `pdf-toolkit/pdf_toolkit_ui.js` | Khung màn hình `#screen-pdf-toolkit`, điều hướng, progress/cancel, dọn bộ nhớ, tích hợp khóa app; export `window.PdfToolkit` |
| `pdf-toolkit/pdf_toolkit_merge.js` | Tool Ghép PDF |
| `pdf-toolkit/pdf_toolkit_pages.js` | Tool Tách PDF + Sắp xếp trang (chung page board, thumbnail lazy) |
| `pdf-toolkit/pdf_toolkit_images.js` | Tool Ảnh thành PDF |
| `pdf-toolkit/pdf_toolkit_pdf2img.js` | Tool PDF thành ảnh |
| `pdf-toolkit/pdf_toolkit_compress.js` | Tool Nén PDF (A: tối ưu giữ chữ / B: nén scan) |
