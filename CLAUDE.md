...
- Phiên bản public: `1.0.0` (nguồn semver duy nhất: `package.json`)
...
### Cache namespace
```js
const VERSION = 'v1.0.0';        // sync-version.mjs quản (regex "VERSION = 'v<semver>'")
...
### Chiến lược fetch
- Navigation: stale-while-revalidate + fallback `caches.match('./index.html')`.
...
- Same-origin khác: network-first. Tiles/CDN: SWR vào cache riêng. OSRM: không intercept.
...
Đang dùng: `__openFolderSeq`, `__galleryLoadSeq`, `__editCustModalSeq`, `__editAssetModalSeq`, `__cameraOpenSeq`, `__refPriceSeq`, `__customerListLoadToken`. Đóng/reset modal → tăng sequence. Form edit: reset field + khóa nút Lưu ngay, chỉ token mới nhất điền form.
...
- `tests/sw-routing.test.js` + `tests/helpers/load-sw.js` — cacheFirst precache-first, không cache response lỗi, activate dọn cache cũ (kể cả tên lịch sử), install không skipWaiting.
...
- `tests/regressions.test.js` — tripwire tĩnh A1/B1/B5/B6/B8/B9.
