...
**05**: CRUD (`saveCustomer`, `deleteCurrentCustomer`, `toggleCustomerStatus`). **Ghi chú tab Info "xem trước, bấm mới sửa" (v1.5.10)**: `#info-notes` mặc định `readonly` (đặt sẵn trong `index.html`, chống flash-of-editable); vào edit qua nút pencil `#btn-edit-notes` HOẶC tap thẳng textarea (cùng `data-action="enterNotesEditMode"`, guard no-op nếu đã edit); nguồn sự thật duy nhất = thuộc tính DOM `readOnly` (CSS ăn theo `:read-only` — rule cạnh block `#info-notes` ở cả `styles.css` lẫn `redesign.clientpro.css`, không class JS-toggle); `exitNotesEditMode()` (nội bộ, KHÔNG có trong `CLICK_ACTIONS`) gọi ở `loadCustomerInfo()` (reset mỗi lần load tab Info — cũng là cơ chế "hủy" edit dở dang, không có nút Cancel riêng) và ở nhánh `putReq.onsuccess` của `saveCustomerNotes()`; mọi đường lỗi/return sớm của save (guard R3, lỗi encryptText, `putReq.onerror`) giữ nguyên edit mode để không mất text đang gõ. Tìm kiếm **không dấu** `_normVi()` (lowercase + bỏ dấu NFD + đ→d) khớp tên + CCCD + SĐT (bỏ khoảng trắng), chỉ số `nName/nPhone/nCccd` cache trong `__custSummaryCache` (không tính lại mỗi keystroke). **Cố ý KHÔNG** dùng virtual list / viewport marker culling (over-engineering ở quy mô vài trăm KH; rAF chunk-render 25 + decrypt cache đủ mượt). `saveCustomerNotes()` có guard R3. `renderFolderHeader` dùng `_displayPlain` (không hiện `cpg1:`). `openFolder()`: sync best-effort rồi slide-in; nền `decryptCustomerSummaryAsync` + notes + TSBĐ (R5) rồi **re-render header + Drive status + tab info/assets**. `openEditCustomerModal` async + `_displayPlainAsync`. Duplicate warning overlay cũng guard + async refresh. Approval: `confirmApproval()`, `closeApproveModal()`.
...
- **Recent (v1.5.10 — Notes UX)**: ô Ghi chú tab Info chuyển sang mô hình "xem trước, bấm mới sửa" — readonly mặc định, nút Sửa (pencil `#btn-edit-notes`) hoặc tap-to-edit vào chế độ gõ, nút Lưu (`#btn-save-notes`) chỉ hiện khi đang edit, tự về readonly sau lưu thành công / khi load lại tab Info (chi tiết §6.4/05).
