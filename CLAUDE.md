...
- Phiên bản public: `1.0.0-hotfix.1` (nguồn semver duy nhất: `package.json`; `sync-version.mjs` chấp nhận hậu tố dạng `-hotfix.N`)
...
`creditLimit` và `asset.name` được mã hóa từ v1.0.0 (trước đó chủ đích plaintext). Migration một lần `runFieldEncryptMigrationV2IfNeeded()` (`02_security.js`, marker `app_field_encrypt_v2_done`) chạy sau unlock: encrypt → **đọc lại xác minh** (kể cả xác minh kết quả THẬT SỰ là ciphertext — chống race lockApp giữa migration) → batch put một transaction; record lỗi giữ nguyên; marker chỉ set khi 100% sạch (lần unlock sau tự retry). Ghi mới: `_encryptCreditLimitForWrite()` (`05_customers.js`), helper `enc()` trong `_doSaveAsset` (`06_assets.js`) — đều mã hóa TRƯỚC khi mở transaction và throw nếu không mã hóa được (không bao giờ ghi plaintext im lặng).
...
1. **Ghi:** `await encryptText(plaintext)` trong `try/catch`, xong trước khi mở transaction. `encryptText()` từ chối chuỗi đã giống ciphertext (chống double-encrypt) và **fail-open trả nguyên plaintext khi không có khóa** — code ghi DB phải kiểm kết quả bằng `_looksEncrypted` nếu ngữ cảnh có thể bị khóa giữa chừng.
...
### Cache namespace
```js
const VERSION = 'v1.0.0-hotfix.1';  // sync-version.mjs quản (semver + hậu tố hotfix hợp lệ)
...
Đang dùng: `__openFolderSeq`, `__galleryLoadSeq`, `__editCustModalSeq`, `__editAssetModalSeq`, `__cameraOpenSeq`, `__refPriceSeq` (guard CẢ lần render đầu của modal tham khảo giá lẫn bước OSRM; `closeRefModal()` tăng seq để hủy kết quả về muộn), `__customerListLoadToken`. Đóng/reset modal → tăng sequence. Form edit: reset field + khóa nút Lưu ngay, chỉ token mới nhất điền form.
...
- **Mọi** promise/callback bọc transaction ghi — kể cả ngoài luồng destructive (`_doSaveCustomer`, `persistCurrentCustomer`, `_idbPutBackup`/`_idbDeleteBackup`) — phải wire cả `onabort`: transaction có thể abort mà KHÔNG có request error đi trước (quota, versionchange...), khi đó `onerror` không bắn; thiếu `onabort` là promise treo và in-flight guard kẹt vĩnh viễn tới khi reload. Callback kiểu `onDone` phải chốt gọi đúng một lần (request error bubble lên `tx.onerror` rồi tx abort bắn tiếp `onabort` — hai sự kiện cho một thất bại).
...
- Drive folder name (`07_drive.js`): dựng từ decrypt async THẬT; không giải mã được → dừng + báo lỗi, không bao giờ đưa ciphertext/rỗng vào tên folder.
...
- `tests/regressions.test.js` — tripwire tĩnh A1/B1/B5/B6/B8/B9 + hotfix.1 #2–#6 (seq guard tham khảo giá, onabort, export fail-closed, báo lỗi inbox restore).
...
- `v1.0.0-hotfix.1` (2026-07-12) — sửa 6 bug từ đợt audit tay sau public release (không bug nào bị test cũ bắt được; đã bổ sung test có mục tiêu):
  1. `sw.js`: `networkFirst()` và bước revalidate của `navigationStaleWhileRevalidate()` chỉ cache khi `res.ok` — lỗi 5xx thoáng qua không còn ghi đè app shell tốt (mục 8).
  2. `06_assets.js`: seq guard cho CẢ lần render đầu của modal tham khảo giá; `closeRefModal()` tăng `__refPriceSeq` (mục 12).
  3. `05_customers.js` (`_doSaveCustomer`, 2 transaction ghi) + `04_ui_common.js` (`persistCurrentCustomer`, kèm settled guard cho `onDone`): wire `onabort` — hết kẹt `__custWriteInFlight`/`__assetSaveInFlight` vĩnh viễn khi tx abort không kèm request error (mục 12).
  4. `12_backup_core.js`: export fail-closed — `_assertUnlockedForExport()` trong `normalizeCustomerForExport()` (trước + sau decrypt) và đầu `exportAll()`/`exportCustomersByIds()`; lock giữa chừng không còn ghi ciphertext im lặng vào backup (mục 7).
  5. `14_cloud_transfer.js`: `CloudTransferUI.acceptAndRestore()` catch + `ErrorHandler.showError` — lỗi inbox restore không còn bị nuốt thành unhandled rejection.
  6. `09_backup_manager.js`: `_idbPutBackup`/`_idbDeleteBackup` wire `onabort` — hết kẹt `__backupInFlight` vĩnh viễn.
  Cache namespace: `clientpro-e2-*-v1.0.0-hotfix.1` (tên mới, chưa từng tồn tại → không bump epoch). `ASSET_V` giữ nguyên (precache install dùng `cache:'reload'` nên asset vẫn tươi). Tooling version (`sync-version.mjs`, ci.yml, `pwa.test.js`) chấp nhận hậu tố hotfix; badge README tự escape `-` thành `--` theo chuẩn shields.io.
