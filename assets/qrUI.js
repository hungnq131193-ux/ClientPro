...
// ---- QR Transfer UI <-> Decoder wiring (FINAL PATCH) ----
window.QRTransferUI = {
  onReceiveProgress: function(info){
    const el = document.getElementById('qr-transfer-progress');
    if(el) el.textContent = 'Đang nhận QR: ' + info.received + '/' + info.total;
  },
  onReceiveComplete: function(out){
    window.__QR_TRANSFER_PAYLOAD__ = out.data;
    const el = document.getElementById('qr-transfer-preview');
    if(el){
      el.textContent = 'Preview: ' + JSON.stringify(out.meta || {}, null, 2);
    }
    const btn = document.getElementById('qr-transfer-restore-btn');
    if(btn) btn.disabled = false;
  },
  onReceiveError: function(err){
    alert('QR Transfer lỗi: ' + err);
  }
};

window.confirmQrRestore = async function(){
  if(!window.__QR_TRANSFER_PAYLOAD__) return alert('Chưa có dữ liệu');
  await window.QRTransferDecode.restoreTransactional(window.__QR_TRANSFER_PAYLOAD__);
