...
function renderFolderHeader(data) {
    getEl('folder-customer-name').textContent = data.name; getEl('folder-avatar').textContent = data.name.charAt(0).toUpperCase(); getEl('btn-detail-call').href = getTelLink(data.phone); getEl('btn-detail-zalo').href = getZaloDeepLink(data.phone); getEl('btn-detail-zalo').onclick = () => { openZaloChat(data.phone); return false; };
...
function showAppInfo() {
    alert('Client Pro\nPhiên bản 2.2.1-ui-vietinbank-light\nPWA nội bộ quản lý khách hàng.');
}
