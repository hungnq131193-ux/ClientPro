...
async function ensureBackupSecret() {
  ...
  try {
    ...
    // Server có thể trả JSON hoặc plain text (do Apps Script). Ta hỗ trợ cả hai.
    let result;
    try {
      result = JSON.parse(txt);
    } catch (e) {
    ...
    // Fallback: parse secret/status/message từ plain text nếu không phải JSON
    let parsedSecret = "";
    let parsedStatus = "";
    let parsedMessage = "";
    if (!result) {
      try {
        const mSecret = txt.match(/"secret"\s*:\s*"([^"]+)"|secret\s*[:=]\s*([A-Za-z0-9_\-\.]+)/i);
        parsedSecret = mSecret ? (mSecret[1] || mSecret[2] || "") : "";

        const mStatus = txt.match(/"status"\s*:\s*"([^"]+)"|status\s*[:=]\s*([A-Za-z_\-\.]+)/i);
        parsedStatus = mStatus ? String(mStatus[1] || mStatus[2] || "") : "";

        const mMsg = txt.match(/"message"\s*:\s*"([^"]+)"|message\s*[:=]\s*"([^"]+)"/i);
        parsedMessage = mMsg ? String(mMsg[1] || mMsg[2] || "") : "";
      } catch (e) {
        parsedSecret = "";
        parsedStatus = "";
        parsedMessage = "";
    ...
    const statusLower = (result && typeof result === "object" && result.status)
      ? String(result.status).toLowerCase()
      : String(parsedStatus || "").toLowerCase();

    const messageFinal = (result && typeof result === "object" && result.message)
      ? result.message
      : parsedMessage;
    ...
    if (statusLower === "locked") {
      ...
      const titleEl = document.getElementById("activation-title");
      if (titleEl)
        titleEl.textContent = messageFinal || "Tài khoản đã bị thu hồi!";
      return {
        ok: false,
        message: messageFinal || "Tài khoản đã bị thu hồi.",
      };
    }
    ...
    if (parsedSecret) {
      APP_BACKUP_SECRET = parsedSecret;
      return { ok: true };
    }
