...
// =======================
// PERF: ASYNC / CACHED DECRYPT HELPERS
// =======================
// CryptoJS decrypt là đồng bộ và chạy trên main-thread. Các helper bên dưới cho phép
// "yield" theo batch để tránh block UI khi danh sách/TSBĐ lớn.

function _yieldToMain() {
  return new Promise((resolve) => {
    try {
      // rAF x2: ưu tiên nhường 1 frame render trước khi tiếp tục
...
function _isProbablyCryptoJSCipher(str) {
  // CryptoJS AES.encrypt(passphrase).toString() thường tạo base64 OpenSSL salted: "U2FsdGVkX1..."
  // Trả false với plaintext để tránh decrypt lặp.
  if (str === undefined || str === null) return false;
  const s = String(str);
  return s.startsWith('U2FsdGVkX1') || s.length > 32; // heuristic nhẹ
}

async function decryptCustomerAssetsAsync(cust, opts = {}) {
  if (!cust || !cust.assets || !Array.isArray(cust.assets) || cust.assets.length === 0) return cust;
  const batchSize = Number.isFinite(opts.batchSize) ? opts.batchSize : 6;
  for (let i = 0; i < cust.assets.length; i++) {
    const a = cust.assets[i];
    if (a) {
      // Cache decrypted fields để renderAssets/openAssetGallery không phải decrypt lại.
      // Không thay đổi schema lưu DB (chỉ tồn tại trong RAM).
      a.__dec = a.__dec || {};
      if (a.__dec._done !== true) {
        a.__dec.name = _isProbablyCryptoJSCipher(a.name) ? decryptText(a.name) : (a.name || '');
        a.__dec.link = _isProbablyCryptoJSCipher(a.link) ? decryptText(a.link) : (a.link || '');
        a.__dec.valuation = _isProbablyCryptoJSCipher(a.valuation) ? decryptText(a.valuation) : (a.valuation || '');
        a.__dec.loanValue = _isProbablyCryptoJSCipher(a.loanValue) ? decryptText(a.loanValue) : (a.loanValue || '');
        a.__dec.area = _isProbablyCryptoJSCipher(a.area) ? decryptText(a.area) : (a.area || '');
        a.__dec.width = _isProbablyCryptoJSCipher(a.width) ? decryptText(a.width) : (a.width || '');
        a.__dec.onland = _isProbablyCryptoJSCipher(a.onland) ? decryptText(a.onland) : (a.onland || '');
        a.__dec.year = _isProbablyCryptoJSCipher(a.year) ? decryptText(a.year) : (a.year || '');
        a.__dec.ocrData = _isProbablyCryptoJSCipher(a.ocrData) ? decryptText(a.ocrData) : (a.ocrData || '');
        a.__dec.driveLink = _isProbablyCryptoJSCipher(a.driveLink) ? decryptText(a.driveLink) : (a.driveLink || '');
        a.__dec._done = true;
      }
    }

    // yield theo batch để không block UI
    if ((i + 1) % batchSize === 0) {
...
async function decryptCustomerObjectAsync(cust, opts = {}) {
  if (!cust) return cust;

  // Summary trước (nhanh)
  cust.name = _isProbablyCryptoJSCipher(cust.name) ? decryptText(cust.name) : cust.name;
  cust.phone = _isProbablyCryptoJSCipher(cust.phone) ? decryptText(cust.phone) : cust.phone;
  cust.cccd = _isProbablyCryptoJSCipher(cust.cccd) ? decryptText(cust.cccd) : cust.cccd;
  cust.driveLink = _isProbablyCryptoJSCipher(cust.driveLink) ? decryptText(cust.driveLink) : cust.driveLink;

  // Sau đó decrypt assets theo batch (nhường frame)
  await decryptCustomerAssetsAsync(cust, opts);
  return cust;
}

// Expose để các module khác có thể dùng (không phá backward-compat)
window.decryptCustomerObjectAsync = decryptCustomerObjectAsync;
window.decryptCustomerAssetsAsync = decryptCustomerAssetsAsync;
